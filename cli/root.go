// Package cli assembles the usacoguide command tree from the usacoguide
...
// NewApp assembles the kit application from the usacoguide domain. The
...
//
// To add a command, declare it in usacoguide/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := usacoguide.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(usacoguide.Domain{}).Register(app)

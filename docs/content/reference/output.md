...
describes what users get. Pick a format with `-o`, or let usacoguide choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
usacoguide <command> -o table   # aligned columns for reading
usacoguide <command> -o jsonl   # one JSON object per line, for piping
usacoguide <command> -o json    # a single JSON array
usacoguide <command> -o csv     # spreadsheet friendly
usacoguide <command> -o tsv     # tab-separated
usacoguide <command> -o url     # just the URL column
usacoguide <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
usacoguide <command> --fields id,title,url
```
...
```bash
usacoguide <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
usacoguide <command>            # a table, because this is a terminal
usacoguide <command> | wc -l    # JSONL, because this is a pipe
```
